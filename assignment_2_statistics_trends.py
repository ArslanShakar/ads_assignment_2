...
# Defining the figure directory path, in which store the plot images.
figure_dir = 'graphs'
...
def load_and_transform_data(filepath):
    """
    This function first load the data from a dataset csv file using
    the pandas library. Extract the data, transposed the original
    dataframe and then  cleaned the transposed dataframe. After that
    it will convert the year column values to numeric.
    :param filepath:
    ...
    index_cols = ['Country Name', 'Country Code',
    ...
    df_transposed['Year'] = pd.to_numeric(
        df_transposed['Year'], errors='coerce')
    return df_original, df_transposed


def extract_specific_data(df_original, countries, indicators,
                          start_year, end_year):
    """
    ...
    cols = ['Country Name', 'Country Code', 'Indicator Name',
            'Indicator Code']
    return filtered_df[cols + years]


def urban_population_vs_forest_area():
    """
    This function first load and extract the data, transposed
    the original dataframe and then cleaned the transposed dataframe.
    After that it will extract the data for specific years. This function
    will show the urban population vs forest area for countries Brazil
    and Canada for the time period 2012 to 2021 by using line plot.
    :return: None
    """
    ...
    specific_data = extract_specific_data(
        df_original, countries, indicators, start_year, end_year)
    print(specific_data)
    ...
    years = [str(year) for year in range(start_year, end_year + 1)]
    specific_data[years] = specific_data[years].apply(
        pd.to_numeric, errors='coerce')

    # Creating separate DataFrames for each indicator
    urban_population_data = specific_data[specific_data['Indicator Code']
                                          == 'EN.URB.MCTY.TL.ZS']

    # describe the urban population dataframe
    print(urban_population_data.describe())

    forest_area_data = specific_data[specific_data['Indicator Code']
                                     == 'AG.LND.FRST.ZS']
    ...
    for country in urban_population_data['Country Code'].unique():
        country_condition = urban_population_data['Country Code'] == country
        country_data = urban_population_data[country_condition]
    ...
    for country in forest_area_data['Country Code'].unique():
        area_condition = forest_area_data['Country Code'] == country
        country_data = forest_area_data[area_condition]
        plt.plot(years, country_data[years].values.flatten(),
                 marker='s', label=f'Forest Area {countries[country]}')

    # Adding titles and labels
    plt.title(f'Urban Population vs Forest Area ({start_year}-{end_year})')
...
def average_urban_population_vs_average_forest_area():
    """
    This function first load and extract the data, transposed the original
    dataframe and then cleaned the transposed dataframe. This function will
    show the average urban population vs average forest area by
    using bar plots.
    :return: None
    """
    ...
    urban_population_avg = urban_population_data.groupby(
        'Country Code')[years].mean().mean(axis=1)
    forest_area_avg = forest_area_data.groupby(
        'Country Code')[years].mean().mean(axis=1)
    # Plotting Bar Charts
    ...
    axes[0].set_title(
        'Average Urban Population ({}-{})'.format(start_year, end_year))
    axes[0].set_ylabel('Percentage')
    # Forest Area Bar Chart
    forest_area_avg.plot(kind='bar', ax=axes[1], color=['red', 'orange'])
    axes[1].set_title(
        'Average Forest Area ({}-{})'.format(start_year, end_year))
    ...
    # Save the plot image to `figures` directory/
    plt.savefig(
        f"{figure_dir}/Average Urban Population vs Average Forest Area")
    # Show plot
    plt.show()
...
def show_stacked_area_plot_indicators():
    """
    This function first load and extract the data, transposed the original
    dataframe and then cleaned the transposed dataframe.  This function
    will generate stacked plots for showing indicators for countries
    Brazil & Canada, during the specific time period 2012 to 2021.
    :return: None
    """
    # Load and extract specific data
    df_original, df_transposed = load_and_transform_data(file_path)
    ...
    # Creating separate DataFrames for each indicator
    urban_population_df = specific_data[specific_data['Indicator Code']
                                        == 'EN.URB.MCTY.TL.ZS']
    forest_area_df = specific_data[specific_data['Indicator Code']
                                   == 'AG.LND.FRST.ZS']
    ...
    for country in countries:
        pop_condition = urban_population_df['Country Code'] == country
        country_urban_data = urban_population_df[pop_condition][years].mean(
            axis=0)
        forest_condition = forest_area_df['Country Code'] == country
        country_forest_data = forest_area_df[forest_condition][years].mean(
            axis=0)
        stacked_data[country] = pd.DataFrame(
            {'Urban Population': country_urban_data,
             'Forest Area': country_forest_data})

    # Plotting Stacked Area Charts
    for country, data in stacked_data.items():
        # create new figure with, set size width is 10 & height is 6 inches.
        # The `dpi` parameter shows dots per inch or resolution for the figure
        plt.figure(figsize=(10, 6), dpi=144)
        plt.stackplot(data.index,
                      data['Urban Population'],
                      data['Forest Area'],
                      labels=['Urban Population', 'Forest Area'],
                      alpha=0.8)

        # Define plot title
        title = (f'Stacked Area Chart of Indicators for '
                 f'{countries[country]} ({start_year}-{end_year})')
        plt.title(title)
        # Set x-axis label
        plt.xlabel('Year')
        # Set y-axis label
...
def show_corr_urban_pop_vs_forest_area():
    """
    This function first load and extract the data, transposed the
    original dataframe and then cleaned the transposed dataframe.
    This function will show the correlation between urban population
    and forest area by making the heatmap.
    :return: None
    """
    ...
    for country in countries:
        u_con = urban_population_data['Country Code'] == country
        f_con = forest_area_data['Country Code'] == country
        urban_avg = urban_population_data[u_con][years].mean(axis=1).values[0]
        forest_avg = forest_area_data[f_con][years].mean(axis=1).values[0]
        # Append country in correlations list Country 
        correlations['Country'].append(country)
        # Append Average Urban Population
        correlations['Urban Population'].append(urban_avg)
        # Append Average Forest Area
    ...
    title = 'Correlation Heatmap between Urban Population and Forest Area'
    plt.title(title)
    # Save the plot image to `figures` directory/
    plt.savefig(f"{figure_dir}/{title}")
    plt.show()
...
if __name__ == "__main__":
    ...
    # Call a function for showing Average Urban Population vs 
    # Average Forest Area
    average_urban_population_vs_average_forest_area()

    # Call a function for showing Stacked Area Chart of Indicators
    # for countries Brazil and Canada
    show_stacked_area_plot_indicators()

    # Call a function for showing Correlation Heatmap between
    # Urban Population and Forest Area
